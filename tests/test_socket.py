...
async def test_arecv(create_bound_pair):
    ...
    async with b, a:
        f1 = b.arecv()
        f2 = b.arecv()
        assert f1.status == Future.Status.PENDING
        assert f2.status == Future.Status.PENDING
        await a.asend_multipart([b"hi", b"there"]).wait()
        recvd = await f2
        assert f1.status != Future.Status.PENDING
        assert f1.return_value == b"hi"
...
async def test_arecv_send(create_bound_pair):
    ...
    async with b, a, create_task_group() as tg:
        ...
        async def callback(future: Future) -> None:
            await future.wait()
            b.send(b", World!")

        tg.start_soon(callback, f)
        a.send(b"Hello")
...
async def test_recv_json_cancelled(push_pull):
    ...
    async with b, a:
        ...
        with pytest.raises(FutureCancelled):
            recvd = await f
        assert f.status == Future.Status.CANCELLED
...
async def test_recv_cancel(push_pull):
    ...
    async with b, a:
        ...
        await a.asend_multipart([b"hi", b"there"]).wait()
        recvd = await f2
        assert f1.status == Future.Status.CANCELLED
        assert f2.status != Future.Status.PENDING
        assert recvd == [b"hi", b"there"]
